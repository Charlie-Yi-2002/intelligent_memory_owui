...
def profile_rewrite(user_id, message):
        ...
        try:
                PROFILE_IP = os.environ.get("PROFILE_IP", "localhost")
                url = f"http://{PROFILE_IP}:8000/persona/ingest_and_append"
                response = requests.post(url, json=payload)
                response.raise_for_status()
                data = response.json()
                return data['persona_query']
        except:
                return message
...
async def process_chat_payload(request, form_data, user, metadata, model):
    ...
    if messages and messages[-1].get("role") == "user":
        log.info(f"Payload {form_data['messages'][-1]} Metadata user_id {metadata['user_id']} chat_id {metadata['chat_id']} message_id {metadata['message_id']}")
        messages[-1]['content'] = profile_rewrite(metadata['user_id'], messages[-1]['content'])
        log.info(f"Rewritten message: {messages[-1]['content']}")
...
async def process_chat_response(
    ...
    if not isinstance(response, StreamingResponse):
        # Log the message in choices
        choices = response.get("choices", [])
        if choices and choices[0].get("message", {}).get("content"):
            content = response["choices"][0]["message"]["content"]
            log.info(f"Response choice: {content} Metadata user_id {metadata['user_id']} chat_id {metadata['chat_id']} message_id {metadata['message_id']}")
        if event_emitter:
    ...
    if event_emitter and event_caller:
        ...
        async def post_response_handler(response, events):
            ...
            if response.background is not None:
                await response.background()
            log.info(f"Response streaming: {serialize_content_blocks(content_blocks)} Metadata user_id {metadata['user_id']} chat_id {metadata['chat_id']} message_id {metadata['message_id']}")
